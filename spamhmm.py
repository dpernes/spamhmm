...
class SpaMHMM(BaseEstimator):                                 
  def __init__(self, n_nodes, mix_dim, n_components, n_features, graph=None,
                ...
                lr_mstep=1e-3, rho1=0.9, rho2=0.99, verbose=False, name='spamhmm'):
  ...
  def init_params(self):
    ...
    transProb = np.exp(np.random.randn(self.mix_dim, self.n_components, self.n_components))
  ...
  def scores_per_seq(self, X, y, lengths=None):
    ...
    for seq_idx, (i,j) in enumerate(iter_from_X_lengths(X, lengths)):
      ...
      log_likelihood[seq_idx] = logsumexp(np.log(self.mixCoef[y[seq_idx], nonzero_idx]) 
  ...
  def _compute_sufficient_statistics_in_mix_comp(self, X, y, lengths, logmixpost, stats):
    ...
    for m in range(self.mix_dim):
      for seq_idx, (i,j) in enumerate(iter_from_X_lengths(X, lengths)):
        ...
        _, fwdlattice = self.mixModels[m]._do_forward_pass(framelogprob)
        bwdlattice = self.mixModels[m]._do_backward_pass(framelogprob)
        posteriors = self.mixModels[m]._compute_posteriors(fwdlattice, bwdlattice)
        ...
        self.mixModels[m]._accumulate_sufficient_statistics(
          stats['mix_idx' + str(m)], X[i:j, :], framelogprob, posteriors,
  ...
  def _compute_sufficient_statistics(self, X, y, lengths):
    ...
    for k in range(self.n_nodes):
      stats['mix_post'][k, :] = np.sum(np.exp(logmixpost[y == k, :]), axis=0)
    
    logmixpost -= np.amax(logmixpost, axis=0).reshape(1, self.mix_dim)
    
    self._compute_sufficient_statistics_in_mix_comp(X, y, lengths, logmixpost, stats)
  ...
  def _adam(self, w, dw, delta=1e-8):
    ...
    Inputs:
      ...
      dw - np.array with the same shape as w, the gradient of the objective
           with respect to w.
    ...
    correct_second_moment = next_second_moment/(1-self.rho2**self.time_)
  ...
  def greedy_predict_seq(self, X, y, x_candidates, n_samples):
    ...
    for i in range(n_samples):
      next_observ = self.predict_next_observ(Xpred, y, x_candidates).reshape(1, -1)
