...
class KHMM(BaseEstimator):                                 
  def __init__(self, n_nodes, n_components, n_features, emission='gaussian',
    ...
    self.hmm = [SpaMHMM(n_nodes=1,
                        ...
                        name=self.name+'_'+str(i)) for i in range(self.n_nodes)]
    ...
    for k in range(self.n_nodes):
      ...
      if valid_data is not None:
        Xk_valid = np.concatenate([X_valid[i:j, :] 
                     for seq_idx, (i,j) in enumerate(iter_from_X_lengths(X_valid, lengths_valid))
                     if y_valid[seq_idx]==k], axis=0)
        lengthsk_valid = [lengths_valid[i] for i in range(len(lengths_valid)) if y_valid[i]==k]
        yk_valid = np.array([0 for i in range(len(lengthsk_valid))])
      
        trainlossk, validlossk = self.hmm[k].fit(Xk, yk, lengths=lengthsk, valid_data=(Xk_valid, yk_valid, lengthsk_valid))
  ...
  def scores_per_seq(self, X, y, lengths=None):
    ...
    for seq_idx, (i,j) in enumerate(iter_from_X_lengths(X, lengths)):
      log_likelihood[seq_idx] = self.hmm[y[seq_idx]].scores_per_seq(X[i:j, :], 
                                                       np.array([0]))
